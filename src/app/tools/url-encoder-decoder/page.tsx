    ...
    <div className="max-w-4xl mx-auto space-y-12">
        ...
        <section>
          ...
          <Card className="prose prose-sm max-w-none text-foreground p-6">
              <p>The tool provides a seamless, real-time experience for converting text to a URL-safe format and back.</p>
              <ol>
                  <li>
                    <strong>Live Conversion:</strong> The tool works instantly. Start typing in either the "Decoded" or "Encoded" box. The other box will automatically update with the converted text.
                  </li>
                  <li>
                    <strong>To Encode:</strong> Type your plain text (e.g., "blue & green widgets?") into the top "Decoded" box. The URL-safe version (e.g., "blue%20%26%20green%20widgets%3F") will appear in the bottom "Encoded" box.
                  </li>
                  <li>
                    <strong>To Decode:</strong> Paste your encoded string (e.g., "search%20results") into the bottom "Encoded" box. The human-readable version ("search results") will appear in the top "Decoded" box.
                  </li>
                  <li>
                    <strong>Swap & Copy:</strong> Use the `Swap` button (<ArrowRightLeft className="inline h-4 w-4" />) to instantly switch the contents of the two boxes. Use the `Copy` icon (<Copy className="inline h-4 w-4" />) above either box to copy its content to your clipboard.
                  </li>
              </ol>
          </Card>
        </section>
        ...
        <Card className='bg-secondary/30 border-primary/20'>
          <CardHeader>
              ...
              <CardDescription>From spaces to special symbols, learn why some characters can't travel freely on the web and how percent-encoding solves the problem.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6 prose prose-lg max-w-none text-foreground">
              ...
              <section>
                  <h3>Percent-Encoding: The Universal Language of URLs</h3>
                  <p>URL encoding, officially known as <strong>percent-encoding</strong>, is a mechanism for translating unsafe characters into a format that is universally safe for transmission. The process replaces an unsafe character with a three-character string:</p>
                  ...
                  <div className="overflow-x-auto my-4">
                    <Table>
                        <TableHeader><TableRow><TableHead>Character</TableHead><TableHead>Encoded</TableHead><TableHead>Reason</TableHead></TableRow></TableHeader>
                        <TableBody>
                            <TableRow><TableCell> </TableCell><TableCell className='font-code'>%20</TableCell><TableCell>Spaces are not allowed in URLs.</TableCell></TableRow>
              ...
              <section>
                  ...
                  <p>JavaScript provides two main functions for this purpose, and it's crucial to know which one to use. This tool uses `encodeURIComponent()`.</p>
                  <ul className='list-disc pl-5'>
                    <li><strong>`encodeURIComponent()`:</strong> This function is aggressive. It assumes you are encoding a piece of a URL, like a query parameter's value or a path segment. It encodes all characters that have special meaning, including `&, +, =, /, ?`. You should use this when building a URL from parts, for example: `let url = "https://example.com/search?q=" + encodeURIComponent("c++ vs c#");`</li>
                    <li><strong>`encodeURI()`:</strong> This function is less aggressive. It assumes you are passing it a full, valid URI and you don't want to break its structure. Therefore, it does **not** encode characters like `?`, `&`, and `/`.</li>
                  </ul>
              </section>
          </CardContent>
        </Card>
            ...
            <Card>
                ...
                <CardContent>
                     <ul className="list-disc pl-5 space-y-3 text-sm text-muted-foreground">
                        ...
                        <li><strong>Encoding the Full URL:</strong> Using `encodeURIComponent()` on an entire URL (e.g., `https://example.com/path?q=test`). This will break the URL by encoding the colons and slashes (`https%3A%2F%2Fexample.com...`), making it unusable.</li>
                    ...
                    <Accordion type="single" collapsible className="w-full">
                        {faqData.map((item, index) => (
                            <AccordionItem value={`item-${index}`} key={index}>
                                ...
                                <AccordionContent>
                                    <div dangerouslySetInnerHTML={{ __html: item.answer.replace(/`([^`]+)`/g, "<code class='font-code bg-muted p-1 rounded-sm'>$1</code>").replace(/<a href='([^']*)' class='[^']*'>/g, "<a href='$1' class='text-primary hover:underline'>") }} />
                                </AccordionContent>
                            </AccordionItem>
                        ))}
                    </Accordion>
