...
const faqData = [
    ...
    { question: "Can I encode an entire file with this tool?", answer: "This tool is designed for text strings. While you could paste the content of a small text file, it's not suitable for binary files like images or executables. For that, you would typically use <a href='/tools/base64-encoder-decoder' class='text-primary hover:underline'>Base64 encoding</a>." },
    ...
    { question: "Why do I see strange characters like `%C3%A9` in URLs?", answer: "This is how non-ASCII characters (like `é`, `ü`, or `ñ`) are represented. The character is first encoded into its UTF-8 byte sequence, and then each byte in that sequence is percent-encoded. For example, the character `é` is represented by two bytes in UTF-8 (`C3` and `A9`), resulting in the encoded string `%C3%A9`. You can learn more about this with our <a href='/tools/unicode-ascii-converter' class='text-primary hover:underline'>Unicode/ASCII Converter</a>." }
];
