...
export const faqData = [
    { question: "What is log rotation?", answer: "Log rotation is an automated process in server administration that manages large log files. It works by renaming the current log file, creating a new empty one, and deleting the oldest files to prevent them from consuming all available disk space." },
    ...
    { question: "What is a 'postrotate' script?", answer: "Some applications keep a continuous 'handle' on their log file. A `postrotate` script is a command that runs after the log is rotated to signal the application to release the old file handle and start writing to the new, empty log file. This is essential for services like Nginx or Apache." },
