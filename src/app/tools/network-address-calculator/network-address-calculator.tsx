            ...
            <Card className='bg-secondary/30 border-primary/20'>
                ...
                <CardContent className="space-y-6 prose prose-lg max-w-none text-foreground">
                    <section>
                        ...
                        <p>In IPv4 networking, a **network address** (or Network ID) is the very first IP address in a subnet range. Its purpose is to serve as a unique identifier for the entire subnet. Think of it like a ZIP code. A ZIP code doesn't point to a single house, but to an entire neighborhood. Similarly, a network address doesn't identify a single computer, but the entire "digital neighborhood" or subnet where a group of devices resides.</p>
                        <p>This address is reserved and **cannot be assigned to any individual host** (like a computer, server, or printer). Its primary role is to be listed in routing tables. When a router needs to send a packet to an IP address, it uses its routing table to find which network that IP belongs to and then sends the packet in the right direction. The network address is the key that makes this entire system work.</p>
                    </section>
                    <section>
                        <h3 className="font-bold text-xl">The Bitwise AND: How the Calculation Works</h3>
                        <p>The magic behind finding the network address is a simple but powerful binary operation called a **bitwise AND**. To find the network ID, a device performs this operation on its own IP address and its subnet mask.</p>
