...
import { Copy, Check, FileCheck, Loader, AlertTriangle } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';

// Hasher functions using Web Crypto API and a JS MD5 implementation

// A simple MD5 implementation for demonstration.
function md5(buffer: ArrayBuffer) {
    ...
    function convertToWordArray(buf: ArrayBuffer) {
        const view = new Uint8Array(buf);
        const lMessageLength = view.length;
        ...
        while (lByteCount < lMessageLength) {
            ...
            lWordArray[lWordCount] = (lWordArray[lWordCount] | (view[lByteCount] << lBytePosition));
            lByteCount++;
        }
        const lWordCount = (lByteCount - (lByteCount % 4)) / 4;
        lBytePosition = (lByteCount % 4) * 8;
    ...
    const x = convertToWordArray(buffer);
    let a = 0x67452301, b = 0xEFCDAB89, c = 0x98BADCFE, d = 0x10325476;
    let k, AA, BB, CC, DD;
    const S11=7, S12=12, S13=17, S14=22; const S21=5, S22=9, S23=14, S24=20; const S31=4, S32=11, S33=16, S34=23; const S41=6, S42=10, S43=15, S44=21;
...
async function digestMessage(buffer: ArrayBuffer, algorithm: 'SHA-1' | 'SHA-256' | 'SHA-512') {
...
export function FileIntegrityChecker() {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (selectedFile) {
            ...
            setHashes({ md5: null, sha1: null, sha256: null, sha512: null });
            setError('');
        }
    };
                ...
                if (buffer) {
                    try {
                        const [md5Hash, sha1Hash, sha256Hash, sha512Hash] = await Promise.all([
                            Promise.resolve(md5(buffer)),
                            digestMessage(buffer, 'SHA-1'),
                            digestMessage(buffer, 'SHA-256'),
                            digestMessage(buffer, 'SHA-512'),
                        ]);
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Select a file to generate its MD5, SHA-1, SHA-256, and SHA-512 checksums. All processing is done securely in your browser.
                </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                <Button onClick={calculateHashes} disabled={isLoading || !file}>
                    {isLoading ? <Loader className="mr-2 h-4 w-4 animate-spin" /> : <FileCheck className="mr-2 h-4 w-4" />}
                    {isLoading ? 'Calculating...' : 'Generate Checksums'}
                </Button>
                ...
                {(isLoading || Object.values(hashes).some(h => h !== null)) && (
                    <div className="space-y-4 pt-4">
                        <h3 className="font-semibold text-lg">Generated Checksums</h3>
                        {file && <p className="text-sm text-muted-foreground">Results for: <strong>{file.name}</strong> ({ (file.size / (1024*1024)).toFixed(2) } MB)</p>}
