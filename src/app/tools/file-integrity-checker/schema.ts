...
export const faqData = [
    ...
    { question: "How does this tool verify file integrity?", answer: "This tool uses cryptographic hash algorithms (MD5, SHA-1, SHA-256, SHA-512) to compute a checksum for the file you provide. You can then compare this calculated checksum to the one provided by the file's source. If they match exactly, the file's integrity is verified." },
    ...
    { question: "Which hash algorithm should I use?", answer: "You should use whichever hash the software provider offers. However, if multiple options are available, always prefer the most secure one. The current industry standard is SHA-256. SHA-1 and especially MD5 are older and considered cryptographically broken for security purposes, but they are still valid for checking against accidental corruption." },
